...
import { getShenShaForGanZhi } from "@/lib/bazi/shensha"
...
export function BaziPaipan({ onBack, onAIAnalysis }: BaziPaipanProps) {
  ...
  // 神煞计算上下文（以本命四柱为基准，供大运/流年逐柱查神煞）
  const shenShaCtx = {
    yearGan: result.year.gan, dayGan: result.day.gan, monthGan: result.month.gan,
    yearZhi: result.year.zhi, dayZhi: result.day.zhi, monthZhi: result.month.zhi,
  }
    ...
    <div className="min-h-screen bg-[#f8f5f0] flex flex-col">
      ...
      {activeTab === "detail" && (
        <div className="flex-1 bg-white pb-24">
          ...
          {/* 岁运关系（当前大运+流年 与 原局八字 的真实生克合冲） */}
          {(() => {
            const allGans = [result.year.gan, result.month.gan, result.day.gan, result.hour.gan]
            const allZhis = [result.year.zhi, result.month.zhi, result.day.zhi, result.hour.zhi]
            if (currentDayun) { allGans.push(currentDayun.gan); allZhis.push(currentDayun.zhi) }
            if (currentLiunian?.gan) { allGans.push(currentLiunian.gan); allZhis.push(currentLiunian.zhi) }
            const suiYun = getGanZhiRelation(allGans, allZhis)
            return (
              <div className="px-4 py-3 space-y-2 border-t border-[#f0f0f0] text-sm">
                <div className="text-xs text-[#999]">
                  当前大运 {currentDayun?.ganZhi ?? "—"} ・ 流年 {currentLiunian?.ganZhi ?? "—"}
                </div>
                <div className="flex">
                  <span className="text-[#d4af37] w-20 shrink-0">岁运天干：</span>
                  <span className="text-[#333]">{suiYun.tianGan.length ? suiYun.tianGan.join(" | ") : "无"}</span>
                </div>
                <div className="flex">
                  <span className="text-[#d4af37] w-20 shrink-0">岁运地支：</span>
                  <span className="text-[#333]">{suiYun.diZhi.length ? suiYun.diZhi.join(" | ") : "无"}</span>
                </div>
                <div className="flex">
                  <span className="text-[#d4af37] w-20 shrink-0">岁运整柱：</span>
                  <span className="text-[#333]">{suiYun.zhengZhu.length ? suiYun.zhengZhu.join(" | ") : "无"}</span>
                </div>
              </div>
            )
          })()}
          ...
          <div className="border-t border-[#f0f0f0]">
            ...
            <div className="px-4 py-3 space-y-3 text-sm">
              {(result.daYun || []).slice(0, 10).map((dy: { ganZhi: string; gan: string; zhi: string }, i: number) => {
                const isCurrentDy = currentDayun?.ganZhi === dy.ganZhi
                const ssList = getShenShaForGanZhi(dy.gan, dy.zhi, shenShaCtx)
                return (
                  <div key={i} className="flex">
